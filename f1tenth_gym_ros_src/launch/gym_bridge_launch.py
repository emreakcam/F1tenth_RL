...
def generate_launch_description():
    ...
    map_server_node = Node(
        ...
        parameters=[{'yaml_filename': config_dict['bridge']['ros__parameters']['map_path'] + '.yaml'},
                    {'topic': 'map'}, {'frame_id': 'map'}, {'use_sim_time': True}]
    )
    ...
    # --- SENİN EĞİTİM DÜĞÜMÜN (RL AGENT) ---
    # Not: 'package' kısmına kendi paket adını, 'executable' kısmına script adını yaz.
    rl_trainer_node = Node(
        package='f1tenth_gym_ros', 
        executable='ros2_f110_env',
        name='ros2_f110_env',
        output='screen',
        parameters=[{'use_sim_time': True}] # Simülasyon zamanını kullanması önemli
    )
    ...
    ld.add_action(rl_trainer_node) # Kendi düğümünü buraya ekledik
